...
#define CUDA_1D_KERNEL_LOOP(i, n)                            \
    ...
    __global__ void ROIAlignForward(const int nthreads, const float* bottom_data, const float spatial_scale, const int height, const int width,
        ...
        CUDA_1D_KERNEL_LOOP(index, nthreads) {
            ...
            // bottom_rois += n * 6;
            float roi_batch_ind = bottom_rois[n * 6 + 0];
            float roi_start_w = bottom_rois[n * 6 + 1] * spatial_scale;
            float roi_start_h = bottom_rois[n * 6 + 2] * spatial_scale;
            float roi_end_w = bottom_rois[n * 6 + 3] * spatial_scale;
            float roi_end_h = bottom_rois[n * 6 + 4] * spatial_scale;
