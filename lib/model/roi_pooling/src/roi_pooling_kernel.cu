    ...
    const float* bottom_rois, float* top_data, int* argmax_data)
{
    CUDA_KERNEL_LOOP(index, nthreads)
    {
        ...
        // bottom_rois += n * 6;
        int roi_batch_ind = bottom_rois[n * 6 + 0];
        int roi_start_w = round(bottom_rois[n * 6 + 1] * spatial_scale);
        int roi_start_h = round(bottom_rois[n * 6 + 2] * spatial_scale);
        int roi_end_w = round(bottom_rois[n * 6 + 3] * spatial_scale);
        int roi_end_h = round(bottom_rois[n * 6 + 4] * spatial_scale);
