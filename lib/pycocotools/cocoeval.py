...
class COCOeval:
    ...
    def summarize(self, f=None):
        '''
        Compute and display summary metrics for evaluation results.
        Note this functin can *only* be applied on the default parameter setting
        '''
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100, f=None):
            p = self.params
            ...
            f.write(iStr.format(titleStr, typeStr, iouStr, areaStr, maxDetsStr, '%.3f'%(float(mean_s)))+'\n')
            return mean_s

        if not self.eval:
            raise Exception('Please run accumulate() first')
        self.stats = np.zeros((12,))
        self.stats[0] = _summarize(1,f=f)
        self.stats[1] = _summarize(1,iouThr=.5,f=f)
        self.stats[2] = _summarize(1,iouThr=.75,f=f)
        self.stats[3] = _summarize(1,areaRng='small',f=f)
        self.stats[4] = _summarize(1,areaRng='medium',f=f)
        self.stats[5] = _summarize(1,areaRng='large',f=f)
        self.stats[6] = _summarize(0,maxDets=1,f=f)
        self.stats[7] = _summarize(0,maxDets=10,f=f)
        self.stats[8] = _summarize(0,maxDets=100,f=f)
        self.stats[9]  = _summarize(0,areaRng='small',f=f)
        self.stats[10] = _summarize(0,areaRng='medium',f=f)
        self.stats[11] = _summarize(0,areaRng='large',f=f)
